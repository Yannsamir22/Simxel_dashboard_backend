...
app.use(
  cors({
    ...
    // FIX: Added X-Request-Id — required by the frontend api.ts request interceptor.
    // Without it the browser blocks every preflight (OPTIONS) request with a CORS error.
...
app.get("/health", (_req, res) => {
  res.status(200).json({
    ok: true,
    service: "Simxel Cloud Dashboard",
