...
export class PackageController {
  ...
  static async getById(req: OwnerRequest, res: Response): Promise<void> {
    const data = await PackageService.getById(req.businessId!, req.params.id);
    if (!data) { res.status(404).json({ ok: false, error: "Package not found." }); return; }
    res.json({ ok: true, data });
  }
  static async create(req: OwnerRequest, res: Response): Promise<void> {
    ...
    if (!name || price === undefined || !serviceIds?.length) {
      res.status(400).json({ ok: false, error: "name, price and serviceIds are required." }); return;
    }
    const data = await PackageService.create(req.businessId!, { name, price, serviceIds });
    res.status(201).json({ ok: true, data });
  }
  static async update(req: OwnerRequest, res: Response): Promise<void> {
    const data = await PackageService.update(req.businessId!, req.params.id, req.body);
    res.json({ ok: true, data });
  }
  static async delete(req: OwnerRequest, res: Response): Promise<void> {
    await PackageService.delete(req.businessId!, req.params.id);
